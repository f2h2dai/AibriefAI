...
DEFAULT_MAX_X_POST_AGE_HOURS = 72
...
def parse_source_timestamp(value) -> datetime | None:
    if not value:
        return None
...
def x_snowflake_timestamp(candidate: dict) -> datetime | None:
    urls = unique_strings(
        candidate.get("url", ""),
...
def x_post_is_fresh(
    ...
    published = source_published_timestamp(candidate)
    if not published:
        return False
    max_age_hours = max(
        1,
        safe_int(env.get("BREAKING_MAX_X_POST_AGE_HOURS"), DEFAULT_MAX_X_POST_AGE_HOURS),
    )
    age = (now or utc_now()) - published
    return timedelta(minutes=-10) <= age <= timedelta(hours=max_age_hours)


def event_terms(candidate: dict) -> set[str]:
    text = " ".join(
        [
            str(candidate.get("title", "")),
            str(candidate.get("content", "")),
            " ".join(str(term) for term in candidate.get("webswarm_evidence_terms", [])),
        ]
    ).lower()
    return {
        word
        for word in re.findall(r"[a-z0-9\u0600-\u06ff]+", text)
        if len(word) > 2 and word not in STOPWORDS
    }


def event_similarity(left: dict, right: dict) -> float:
    left_urls = set(left.get("source_urls", []))
    right_urls = set(right.get("source_urls", []))
    if left_urls & right_urls:
        return 1.0

    left_terms = event_terms(left)
    right_terms = event_terms(right)
    if not left_terms or not right_terms:
        return 0.0

    shared = left_terms & right_terms
    combined = left_terms | right_terms
    jaccard = len(shared) / max(1, len(combined))

    if "mizarvision" in left_terms and "mizarvision" in right_terms:
        return max(jaccard, 0.9)

    grok_event_markers = {"pentagon", "iran", "munitions", "targets", "maven", "epic", "fury"}
    if "grok" in left_terms and "grok" in right_terms:
        marker_overlap = (left_terms & grok_event_markers) & (right_terms & grok_event_markers)
        if len(marker_overlap) >= 2:
            return max(jaccard, 0.9)

    left_node = str(left.get("webswarm_node", "")).strip()
    right_node = str(right.get("webswarm_node", "")).strip()
    if left_node and left_node == right_node and left_node != "x_intel" and len(shared) >= 4:
    ...
    for raw in raw_candidates:
        ...
        match = next(
            (
                ...
                if existing.get("story_fingerprint") == fingerprint
                or event_similarity(existing, candidate) >= 0.58
            ),
            None,
        )
        if match:
            merge_cluster(match, candidate)
        else:
...
def public_feed_recent(entry: dict, now: datetime | None = None, max_age_hours: int = DEFAULT_PUBLIC_FEED_MAX_AGE_HOURS) -> bool:
    source = public_source_name(str(entry.get("source", "")))
    urls = " ".join(entry.get("source_urls") or [entry.get("source_url", "")]).lower()
    is_x_entry = source == "x" or "x.com/" in urls or "twitter.com/" in urls
    timestamp = source_published_timestamp(entry) if is_x_entry else None
    timestamp = timestamp or parse_source_timestamp(entry.get("alerted_at") or entry.get("last_seen_at"))
    if not timestamp:
        return False
    age = (now or utc_now()) - timestamp
    return timedelta(minutes=-10) <= age <= timedelta(hours=max_age_hours)
...
def public_feed_entries(state: dict, limit: int = 12) -> list[dict]:
    ...
    for fingerprint, entry in state.get("alerted", {}).items():
        ...
        source_published = source_published_timestamp(entry)
        entries.append(
            {
                "id": fingerprint,
                "title": entry.get("title", ""),
                "shown_at": entry.get("alerted_at", ""),
                "source_published_at": isoformat(source_published) if source_published else "",
...
def public_breaking_status(state: dict, summary: dict | None = None) -> dict:
    ...
    return {
        ...
        "alerted_count": len(feed_entries),
        "pending_count": len(pending),
        "feed": feed_entries,
        "pending_feed": pending_entries,
        "last_alert": {
            "title": latest.get("title", "") if latest else "",
            "alerted_at": latest.get("shown_at", "") if latest else "",
            "source_url": latest.get("source_url", "") if latest else "",
...
def normalize_x_record(record: dict, handle: str = "") -> dict | None:
    ...
    return {
        ...
        "published_at": (
            ...
            or record.get("createdAtISO")
            or record.get("created_at_iso")
...
def collect_x_cli(env: dict[str, str], limit: int = 20) -> list[dict]:
    ...
    for query in x_search_queries(env):
        for command in x_search_commands(query):
            ...
            parsed_all = parse_x_cli_output(completed.stdout, handle)
            parsed = [
                candidate
                for candidate in parsed_all
    ...
    *,
    ...
    if notify_mode == "website":
        ...
        summary = {
            ...
            "x_collector_status": "healthy" if initial_x_relevant else "degraded",
